...
Twelve of the thirteen examples ship no image files at all: they build their
...
| `pixel-patience` | Generated at run time, or your own card sheet — see below |
...
with generated art, the way the other twelve examples work, would close the
question outright.

"Generated at run time" means the game builds its sprite sheets as RGBA byte
arrays and hands them to `upload_sheet`. `examples/pixel-patience/src/deck.ts`
is the fullest example: a complete 52-card deck, pips and all, in about 250
lines of pixel arithmetic.

## Bringing your own art

`pixel-patience` is set up to use a licensed 8-bit card sheet if you have one.
Save it as `examples/pixel-patience/playing_cards.png` and the game picks it up
on the next reload; the status line under the canvas says which deck is in
play. The file is in that example's `.gitignore`, so it will not be committed
by accident.
...
`src/table.ts` holds those numbers, so a sheet with different cell dimensions
is a two-line change rather than a rewrite.
...
So the game reads the sheet if it is there and draws its own deck if it is
not. Both paths are covered by the example's headless checks. If you are
