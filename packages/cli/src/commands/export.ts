...
export default async function exportCommand(target: string) {
  if (target === 'web') {
    ...
    try {
      execSync(
        'cargo build -p retro-platform-native --target armv7-unknown-linux-gnueabihf --release',
        { stdio: 'inherit' },
      );
      const binaryName = process.platform === 'win32' ? 'retro-platform-native' : 'retro-platform-native';
...
/* Game Boy ROM scaffold generated by retro-engine CLI */
...
`;
    ...
    const mainC = `/* NES ROM scaffold generated by retro-engine CLI */
